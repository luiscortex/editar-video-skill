"""STUB MVP — gerar_caption_ig. Substituir por skill real em v1.1+."""
import argparse
import json
import shutil
import sys
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="STUB skill gerar_caption_ig (MVP placeholder)")
    ...
    if args.input and Path(args.input).suffix.lower() in {".mp4", ".mov", ".webm"}:
        ...
        out.write_text(json.dumps({
            "_stub": True,
            "_skill": "gerar_caption_ig",
            "_message": "Stub MVP — implementação real em v1.1+",
        }, indent=2))

    print(f"OK (stub): gerar_caption_ig → {out}", file=sys.stderr)
    return 0
